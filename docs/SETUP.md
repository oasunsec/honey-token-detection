...
```text
python -m app.cli create --name "Financial records" --filename "Financial_Records_CONFIDENTIAL.docx" --format docx
```
...
Open the generated file on the machine running the receiver. If the viewer loads external content, its pixel request will appear in `/api/events`. Viewer behavior varies with Office and endpoint policies.
...
```text
[CANARY] Financial_Records_CONFIDENTIAL.docx triggered (HIGH)
```
...

{
  ...
  "filename": "Financial_Records_CONFIDENTIAL.docx",
  "severity": "high",
  "notes": "Sample uses synthetic financial data only."
}
```
