...
| SMTP sink | The loopback sink accepted one SMTP `DATA` message after two callbacks; two events persisted with `sent` and `suppressed` alert states ([test](tests/test_smtp_delivery.py), [transcript](docs/evidence/public/15-evidence.html)) |
